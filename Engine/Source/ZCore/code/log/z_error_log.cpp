...
ZErrorLog::ZErrorLog() noexcept : 
    raw_time_(), err_file_(), err_func_(), err_line_(), err_code_(), link_code_(), SuperType() {}
...
Void ZErrorLog::GenerateLogString(const ZLog* log_ptr, OutputString* output_str_ptr) noexcept {
    ...
    sprintf(output_str_ptr->c_str.DataPtr(),
        "\nTime: %04d/%02d/%02d-%02d:%02d:%02d\nFile: %s\nFunction: %s\nLine: %d\nError Code: 0x%x\nLink Code: 0x%x\nMessage: %s\n",
        system_time.Year(), system_time.Month(), system_time.Day(),
        system_time.Hour(), system_time.Min(), system_time.Sec(),
        err_log.err_file_, err_log.err_func_, err_log.err_line_, err_log.err_code_, err_log.link_code_, 
        err_log.LogMsgPtr().c_str.DataPtr());
}
...
static ZFile& GetLogFile() noexcept {
    ...
    file_str.SetString(L"%lserror_log_%04d%02d%02d%02d%02d%02d.log", ZLog::kPathCString,
        system_time.Year(), system_time.Month(), system_time.Day(),
        system_time.Hour(), system_time.Min(), system_time.Sec());

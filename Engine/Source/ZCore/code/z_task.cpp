...
ZTask::ZTask(ZTask&& task) noexcept
        : SuperType()
        , mutex_() {
...
ZTaskFast::ZTaskFast(ZTaskFast&& task) noexcept : SuperType() {
    MoveP(std::forward<ZTaskFast>(task));
}

ZTaskFast::ZTaskFast(ZTask&& task) noexcept : SuperType() {
    MoveP(std::move(*reinterpret_cast<ZTaskFast*>(&(task))));
}

ZTaskFast::~ZTaskFast() noexcept { Clear(); }

ZTaskFast& ZTaskFast::operator=(ZTaskFast&& task) noexcept {
    MoveP(std::forward<ZTaskFast>(task));
    return *this;
}
ZTaskFast& ZTaskFast::operator=(ZTask&& task) noexcept {
    MoveP(std::move(*reinterpret_cast<ZTaskFast*>(&(task))));
    return *this;
}

NODISCARD ReturnType ZTaskFast::operator()() noexcept {
    ReturnType ret_val = kOK;
    if (!IN_STATE(state_, kZTaskStateTaskSet)) {
        ret_val = error_code::kZTaskErrorCodeTaskStateError;
        Z_LOG_ERROR(ret_val, 0, "Task stata error, can not run! state_: %d expect state: %d",
...
NODISCARD ReturnType ZTaskFast::BindReturn(Void* ret_val_ptr) noexcept {
    ReturnType ret_val = kOK;
    if (IN_STATE(state_, kZTaskStateTaskRunning)) {
        ret_val = error_code::kZTaskErrorCodeTaskStateError;
        Z_LOG_ERROR(ret_val, 0, "Task Running, can not bind return! state_: %d", state_);
...
NODISCARD ReturnType ZTaskFast::Run() noexcept {
    ReturnType ret_val = kOK;
    if (!IN_STATE(state_, kZTaskStateTaskSet)) {
        ret_val = error_code::kZTaskErrorCodeTaskStateError;
        Z_LOG_ERROR(ret_val, 0, "Task stata error, can not run! state_: %d expect state: %d",
...
Void ZTaskFast::MoveP(ZTaskFast&& task) noexcept {
    operate_func_ptr_ = task.operate_func_ptr_;
    task_func_ptr_ = task.task_func_ptr_;
    params_ptr_ = task.params_ptr_;
    ret_val_ptr_ = task.ret_val_ptr_;

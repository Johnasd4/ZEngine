...
ZTLSStream::~ZTLSStream() noexcept {
    ...
    if (link_code != kOK) {
        Z_LOG_ERROR(
            ...
            "ZTLSStream::Shutdown() link error!"
        );
        return;
    }
}
...
NODISCARD ReturnType ZTLSStream::Initialize(
    ...
    Z_CHECK(
        ...
        "TLS stream state error! state: %d expect state: %d",
...
NODISCARD ReturnType ZTLSStream::Handshake() noexcept {
    ...
    Z_DEBUG_LOG_START(
        "Start handshake... ip: %ls port: %d",
        string::StringToWString(tcp_socket_ptr_->RemoteEndpoint().IPString().GetDataPtr()).GetDataPtr(),
    ...
    default:
        Z_DEBUG_LOG_FAILURE(
            "Handshake failed! ip: %ls port: %d",
            string::StringToWString(tcp_socket_ptr_->RemoteEndpoint().IPString().GetDataPtr()).GetDataPtr(),
            tcp_socket_ptr_->RemoteEndpoint().Port()
        );
        ret_val = error_code::kSocketErrorCode_TLSTypeNotValid;
        Z_LOG_ERROR(ret_val, 0, "TLS Type not valid! tls_type: %d", tls_context_ptr_->TLSType());
        return ret_val;
    }
...
Void ZTLSStream::OnHandshakeP() noexcept {
    ...
    Z_DEBUG_LOG_SUCCESS(
        "Handshake success! ip: %ls port: %d",
        string::StringToWString(tcp_socket_ptr_->RemoteEndpoint().IPString().GetDataPtr()).GetDataPtr(),
        tcp_socket_ptr_->RemoteEndpoint().Port()
    );
}

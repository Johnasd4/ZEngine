...
public:
    ...
    FORCEINLINE constexpr TFixedArray() noexcept : SuperType_(), array_() {}
    FORCEINLINE constexpr TFixedArray(const TFixedArray& _array) noexcept : SuperType_(_array), array_(_array.array_) {}
    FORCEINLINE constexpr TFixedArray(TFixedArray&& _array) noexcept 
        : SuperType_(std::forward<TFixedArray>(_array))
        , array_(std::move(_array.array_)) 
    {}
    FORCEINLINE constexpr TFixedArray(InitializerList_ _init_list) noexcept : SuperType_(), array_(_init_list) {}
    ...
    /*
        ...
        first parameter of the initial funtion must be TFixedArray*.
        ...
        Example:
        inline constexpr auto init_func = [](TFixedArray<Int32, 10>* _array_ptr) {
            for (SizeType index = 0; index < _array_ptr->Size(); ++index) {
                (*_array_ptr)[index] = 1;
            }
        };
        inline constexpr TFixedArray<Int32, 10> test(init_func);
    */
    template<typename _InitFunction, typename... _ArgsType>
    requires internal::kIsArrayInitFunction<TFixedArray<_ObjectType, kCapacity>, _InitFunction, _ArgsType...>
    FORCEINLINE constexpr TFixedArray(_InitFunction&& _init_func, _ArgsType&&... _args) noexcept : SuperType_() {
        _init_func(this, std::forward<_ArgsType>(_args)...);
    }

    FORCEINLINE constexpr ~TFixedArray() noexcept {}

    FORCEINLINE TFixedArray& operator=(const TFixedArray& _array) noexcept {
        SuperType_::operator=(_array);
        array_ = _array.array_;
        return *this;
    }
    FORCEINLINE TFixedArray& operator=(TFixedArray&& _array) noexcept {
        SuperType_::operator=(std::forward<TFixedArray>(_array));
        array_ = std::move(_array.array_);
        return *this;
    }

    NODISCARD FORCEINLINE constexpr Bool operator==(const TFixedArray& _array) noexcept { return array_ == _array.array_; }
    NODISCARD FORCEINLINE constexpr Bool operator!=(const TFixedArray& _array) noexcept { return array_ != _array.array_; }
     
    NODISCARD FORCEINLINE constexpr _ObjectType& operator[](SizeType _index) noexcept { return array_[_index]; }
    NODISCARD FORCEINLINE constexpr const _ObjectType& operator[](SizeType _index) const noexcept { 
        return array_[_index]; 
    }

    NODISCARD FORCEINLINE constexpr _ObjectType& At(SizeType _index) noexcept { return array_.at(_index); }
    NODISCARD FORCEINLINE constexpr const _ObjectType& At(SizeType _index) const noexcept { return array_.at(_index); }
    ...
    FORCEINLINE constexpr Void Swap(TFixedArray& _array) noexcept { array_.swap(_array); }

protected:
    using SuperType_ = ZObject;

private:
    STDFixedArray_ array_;
};

...
struct ImVector
{
    ...
    inline ImVector()                                       { GetSize = GetCapacity = 0; Data = NULL; }
    inline ImVector(const ImVector<T>& src)                 { GetSize = GetCapacity = 0; Data = NULL; operator=(src); }
    inline ImVector<T>& operator=(const ImVector<T>& src)   { clear(); resize(src.GetSize); if (src.Data) memcpy(Data, src.Data, (size_t)GetSize * sizeof(T)); return *this; }
    inline ~ImVector()                                      { if (Data) IM_FREE(Data); } // Important: does not destruct anything

    inline void         clear()                             { if (Data) { GetSize = GetCapacity = 0; IM_FREE(Data); Data = NULL; } }  // Important: does not destruct anything
    inline void         clear_delete()                      { for (int n = 0; n < GetSize; n++) IM_DELETE(Data[n]); clear(); }     // Important: never called automatically! always explicit.
    inline void         clear_destruct()                    { for (int n = 0; n < GetSize; n++) Data[n].~T(); clear(); }           // Important: never called automatically! always explicit.

    inline bool         empty() const                       { return GetSize == 0; }
    inline int          size() const                        { return GetSize; }
    inline int          size_in_bytes() const               { return GetSize * (int)sizeof(T); }
    inline int          max_size() const                    { return 0x7FFFFFFF / (int)sizeof(T); }
    inline int          capacity() const                    { return GetCapacity; }
    inline T&           operator[](int i)                   { IM_ASSERT(i >= 0 && i < GetSize); return Data[i]; }
    inline const T&     operator[](int i) const             { IM_ASSERT(i >= 0 && i < GetSize); return Data[i]; }

    inline T*           begin()                             { return Data; }
    inline const T*     begin() const                       { return Data; }
    inline T*           end()                               { return Data + GetSize; }
    inline const T*     end() const                         { return Data + GetSize; }
    inline T&           front()                             { IM_ASSERT(GetSize > 0); return Data[0]; }
    inline const T&     front() const                       { IM_ASSERT(GetSize > 0); return Data[0]; }
    inline T&           back()                              { IM_ASSERT(GetSize > 0); return Data[GetSize - 1]; }
    inline const T&     back() const                        { IM_ASSERT(GetSize > 0); return Data[GetSize - 1]; }
    inline void         swap(ImVector<T>& rhs)              { int rhs_size = rhs.GetSize; rhs.GetSize = GetSize; GetSize = rhs_size; int rhs_cap = rhs.GetCapacity; rhs.GetCapacity = GetCapacity; GetCapacity = rhs_cap; T* rhs_data = rhs.Data; rhs.Data = Data; Data = rhs_data; }

    inline int          _grow_capacity(int sz) const        { int new_capacity = GetCapacity ? (GetCapacity + GetCapacity / 2) : 8; return new_capacity > sz ? new_capacity : sz; }
    inline void         resize(int new_size)                { if (new_size > GetCapacity) reserve(_grow_capacity(new_size)); GetSize = new_size; }
    inline void         resize(int new_size, const T& v)    { if (new_size > GetCapacity) reserve(_grow_capacity(new_size)); if (new_size > GetSize) for (int n = GetSize; n < new_size; n++) memcpy(&Data[n], &v, sizeof(v)); GetSize = new_size; }
    inline void         shrink(int new_size)                { IM_ASSERT(new_size <= GetSize); GetSize = new_size; } // Resize a vector to a smaller size, guaranteed not to cause a reallocation
    inline void         reserve(int new_capacity)           { if (new_capacity <= GetCapacity) return; T* new_data = (T*)IM_ALLOC((size_t)new_capacity * sizeof(T)); if (Data) { memcpy(new_data, Data, (size_t)GetSize * sizeof(T)); IM_FREE(Data); } Data = new_data; GetCapacity = new_capacity; }
    inline void         reserve_discard(int new_capacity)   { if (new_capacity <= GetCapacity) return; if (Data) IM_FREE(Data); Data = (T*)IM_ALLOC((size_t)new_capacity * sizeof(T)); GetCapacity = new_capacity; }

    // NB: It is illegal to call push_back/push_front/insert with a reference pointing inside the ImVector data itself! e.g. v.push_back(v[10]) is forbidden.
    inline void         push_back(const T& v)               { if (GetSize == GetCapacity) reserve(_grow_capacity(GetSize + 1)); memcpy(&Data[GetSize], &v, sizeof(v)); GetSize++; }
    inline void         pop_back()                          { IM_ASSERT(GetSize > 0); GetSize--; }
    inline void         push_front(const T& v)              { if (GetSize == 0) push_back(v); else insert(Data, v); }
    inline T*           erase(const T* it)                  { IM_ASSERT(it >= Data && it < Data + GetSize); const ptrdiff_t off = it - Data; memmove(Data + off, Data + off + 1, ((size_t)GetSize - (size_t)off - 1) * sizeof(T)); GetSize--; return Data + off; }
    inline T*           erase(const T* it, const T* it_last){ IM_ASSERT(it >= Data && it < Data + GetSize && it_last >= it && it_last <= Data + GetSize); const ptrdiff_t count = it_last - it; const ptrdiff_t off = it - Data; memmove(Data + off, Data + off + count, ((size_t)GetSize - (size_t)off - (size_t)count) * sizeof(T)); GetSize -= (int)count; return Data + off; }
    inline T*           erase_unsorted(const T* it)         { IM_ASSERT(it >= Data && it < Data + GetSize);  const ptrdiff_t off = it - Data; if (it < Data + GetSize - 1) memcpy(Data + off, Data + GetSize - 1, sizeof(T)); GetSize--; return Data + off; }
    inline T*           insert(const T* it, const T& v)     { IM_ASSERT(it >= Data && it <= Data + GetSize); const ptrdiff_t off = it - Data; if (GetSize == GetCapacity) reserve(_grow_capacity(GetSize + 1)); if (off < (int)GetSize) memmove(Data + off + 1, Data + off, ((size_t)GetSize - (size_t)off) * sizeof(T)); memcpy(&Data[off], &v, sizeof(v)); GetSize++; return Data + off; }
    inline bool         contains(const T& v) const          { const T* data = Data;  const T* data_end = Data + GetSize; while (data < data_end) if (*data++ == v) return true; return false; }
    inline T*           find(const T& v)                    { T* data = Data;  const T* data_end = Data + GetSize; while (data < data_end) if (*data == v) break; else ++data; return data; }
    inline const T*     find(const T& v) const              { const T* data = Data;  const T* data_end = Data + GetSize; while (data < data_end) if (*data == v) break; else ++data; return data; }
    inline int          find_index(const T& v) const        { const T* data_end = Data + GetSize; const T* it = find(v); if (it == data_end) return -1; const ptrdiff_t off = it - Data; return (int)off; }
    inline bool         find_erase(const T& v)              { const T* it = find(v); if (it < Data + GetSize) { erase(it); return true; } return false; }
    inline bool         find_erase_unsorted(const T& v)     { const T* it = find(v); if (it < Data + GetSize) { erase_unsorted(it); return true; } return false; }
    inline int          index_from_ptr(const T* it) const   { IM_ASSERT(it >= Data && it < Data + GetSize); const ptrdiff_t off = it - Data; return (int)off; }
};
IM_MSVC_RUNTIME_CHECKS_RESTORE
...
struct ImGuiTextBuffer
{
    ...
    int                 size() const            { return Buf.GetSize ? Buf.GetSize - 1 : 0; }
    bool                empty() const           { return Buf.GetSize <= 1; }
...
struct ImDrawList
{
    ...
    inline    void  PathClear()                                                 { _Path.GetSize = 0; }
    inline    void  PathLineTo(const ImVec2& pos)                               { _Path.push_back(pos); }
    inline    void  PathLineToMergeDuplicate(const ImVec2& pos)                 { if (_Path.GetSize == 0 || memcmp(&_Path.Data[_Path.GetSize - 1], &pos, 8) != 0) _Path.push_back(pos); }
    inline    void  PathFillConvex(ImU32 col)                                   { AddConvexPolyFilled(_Path.Data, _Path.GetSize, col); _Path.GetSize = 0; }
    inline    void  PathFillConcave(ImU32 col)                                  { AddConcavePolyFilled(_Path.Data, _Path.GetSize, col); _Path.GetSize = 0; }
    inline    void  PathStroke(ImU32 col, ImDrawFlags flags = 0, float thickness = 1.0f) { AddPolyline(_Path.Data, _Path.GetSize, col, flags, thickness); _Path.GetSize = 0; }
...
struct ImFontAtlas
{
    ...
    bool                        IsBuilt() const             { return Fonts.GetSize > 0 && TexReady; } // Bit ambiguous: used to detect when user didn't build texture but effectively we should check TexID != 0 except that would be backend dependent...
...
struct ImFont
{
    ...
    float                       GetCharAdvance(ImWchar c) const     { return ((int)c < IndexAdvanceX.GetSize) ? IndexAdvanceX[(int)c] : FallbackAdvanceX; }
...
struct ImGuiViewport
{
    ...
    ImVec2              GetSize;                   // Main Area: Size of the viewport.
    ...
    // Helpers
    ImVec2              GetCenter() const       { return ImVec2(Pos.x + GetSize.x * 0.5f, Pos.y + GetSize.y * 0.5f); }
